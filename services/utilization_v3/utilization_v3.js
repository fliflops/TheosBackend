...
const utilization = async({
    ...
    if(truckList.length > 0){
        availableTrips = bookings.filter(item => {
       ...
       truckNo =  typeof _.maxBy(utilized,'truck_no') === 'undefined' ? 0 : (_.maxBy(utilized,'truck_no')).truck_no    
    }
    else {
        availableTrips = bookings.filter(item => item.Util_factor <= capacity);
    }
...
const sixWheelUtilization = async({
    ...
    try{
        ...
        // console.log(startCapacity)

        let availableBookings = _.orderBy(bookings.filter(x => x.Util_factor <= capacity && (x.long !== null ||x.lat !== null)),'Util_factor','desc');
