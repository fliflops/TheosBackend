...
router.get('/principal', async(req, res) => {
    try{
        ...
        let { PrincipalId_inp } = req.query;

        const ngayon = moment().format('YYYY-MM-DD');

        PrincipalId_inp = typeof(PrincipalId_inp) === 'undefined' || PrincipalId_inp == '' ? '' : PrincipalId_inp;

        // const filter = { 
        //                 'filename' : Filename_inp,
        //                 'order' : PrincipalId_inp,
        //                 'createdFrom' : paramdate1_inp+' 00:00:01',
        //                 'createdTo' : paramDate2_inp+' 23:59:59'
        // }

        const principals = null ; //await bookingService.getBookings(filter)

        res.render('../views/master/principal', {principals, PrincipalId_inp})
    }
    catch(e){
        console.log(e)
...
router.get('/shippoint', async(req, res) => {
    try{

        let { stc_inp } = req.query;

        stc_inp = typeof(stc_inp) === 'undefined' || stc_inp == '' ? '' : stc_inp;

        const shippoints = await masterService.getShipPoints(filter)
        // console.log(principals, 'principals')

        res.render('../views/master/shippoint', {shippoints, stc_inp})
    }
    catch(e){
        console.log(e)
        // res.status(500).json({message:e});
...
router.get('/vehicletype', async(req, res) => {
    try{
        ...
        let { vehtype_inp } = req.query;

        vehtype_inp = typeof(vehtype_inp) === 'undefined' || vehtype_inp == '' ? '' : vehtype_inp;

        const vehtypes = null ; //await bookingService.getBookings(filter)
        // console.log(vehtypes, 'vehtypes')

        res.render('../views/master/shippoint', {vehtypes, vehtype_inp})
    }
    catch(e){
        console.log(e)
        // res.status(500).json({message:e});
